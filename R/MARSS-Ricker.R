#MARSS DLM Ricker Model - Time varying intercept, PDO, and NPGO effects
# MARSS_Ricker <- function(dat.input, covars) {
  require(MARSS)
  ...
  diag(Q) = c(0,"q.pdo","q.npgo") # 2x2; diag = (q1,q2)
  
  #DEFINE OBSERVATION MODEL
  # for observation eqn
  Z = array(NA, c(1,m,TT)) # NxMxT; empty for now
  Z[1,1,] = rep(1,TT) # Nx1; 1's for intercept
  Z[1,2:3,] = pred # Nx1; regr variable
  ...
  mod.list = list(B=B, Q=Q, U=U, Z=Z, A=A, R=R)#, D='unconstrained', d=matrix(1))
  
  #FIT THE MODEL
  # fit univariate DLM
  foo <- MARSS(dat, inits=inits.list, model=mod.list, fit=FALSE)
  dlm <- MARSS(dat, inits=inits.list, model=mod.list, control=list(maxit=1e3), silent=2)
