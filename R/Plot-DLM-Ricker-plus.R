...
dir.figs <- file.path(wd,"Figs","DLM-Ricker-plus")
dir.output <- file.path(wd,"Output","DLM-Ricker-plus")
...
pars <- c('alpha','beta','rho','sigma_oe','sigma_pdo', 'sigma_npgo')
    ...
    for(p in 1:n.stocks) {
      ...
      if(file.exists(file.path(dir.output,paste0(temp.species,'-',temp.stock,'-fit.rds')))) {
        ...
        vals <- rbind(c(mean(ex$alpha), quantile(ex$alpha, probs=c(0.025,0.25,0.5,0.75,0.975))),
                      ...
                      c(mean(ex$rho), quantile(ex$rho, probs=c(0.025,0.25,0.5,0.75,0.975))),
