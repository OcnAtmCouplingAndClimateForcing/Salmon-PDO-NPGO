...
#CONTROL SECTION ==========================================================
read.rds <- TRUE #Whether to read in data from stan output files
...
if(read.rds==TRUE) {
  ...
  for(s in 1:n.species) {
    ...
    for(p in 1:n.stocks) {
      ...
      #Load Data
      if(file.exists(file.path(dir.output,paste0(temp.species,'-',temp.stock,'-out.rds')))) {
        
        out <- readRDS(file=file.path(dir.output,paste0(temp.species,'-',temp.stock,'-out.rds')))
        
        #Extract coefficients
        ex <- out$dlm
        
        err <- MARSSparamCIs(ex)
        ...
        # pars <- c('alpha','beta','sigma_oe','sigma_pdo', 'sigma_npgo')
        # vals <- rbind(ex$par$A[1,1],
        #               ex$par$A[2,1],
        #               
...
pdf(file.path(dir.figs, "Effect Timeseries_stdx.pdf"), height=8, width=10)
s <- 1
for(s in 1:n.species) {
  #PDO
  covar <- 'PDO'
  g <- ggplot(effect.df[effect.df$species==species[s] & 
              ...
              aes(x=covYr, y=est, color=region, group=stock)) +
    theme_linedraw() +
    # scale_color_gdocs() +
...
#Page PArs
pdf(file.path(dir.figs,'Pars_2.pdf'), height=6, width=9)
p <- 1
